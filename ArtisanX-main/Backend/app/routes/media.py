...
general_media_router = APIRouter(
    prefix="/api/media",
    tags=["Media Upload"]
)


@general_media_router.post("/upload")
async def upload_general_image(
    file: UploadFile = File(...),
):
    """
    Direct image upload endpoint for camera photos, gallery uploads,
    profile pictures, and product staging.
    """
    ...
    stored_name = generate_storage_filename(file.filename)
    dest_path = general_dir / stored_name
    ...
    return {
        ...
        "url": f"/uploads/general/{stored_name}",
